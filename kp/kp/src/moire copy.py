...
def compute_eigenvalues(i,k):
    ...
    for term in model.terms.values():
        ...
        if term.active:
            ...
            # print(f"key: {key}")
            Mz,Mz_star,l1,l2,a1,a2,p=key.Mz,key.Mz_star,key.layer_from, key.layer_to, key.orbital_from, key.orbital_to, key.p
            M_sum = Mz + Mz_star
            key = ContinuumTermKey(Mz, Mz_star, l1, l2, a1, a2, p)
            # if l1 == l2 and a1 == a2 and np.sum(np.abs(p)) <0.001 and Mz + Mz_star > 8:              
            #     continue
            # if l1 == l2 :
            #     continue
            # if (a1-a2)%2==1 and np.sum(np.abs(p)) >np.sum(np.abs(intra_harmonics_map[2])) or ((a1-a2)%2==1 and np.sum(np.abs(p)) <0.001 and M_sum > 3):
            #     # if np.sum(np.abs(p)) > np.sum(np.abs(intra_harmonics_map[3])):
            #     continue
            # if M_sum > 2 and np.sum(np.abs(p)) > np.sum(np.abs(intra_harmonics_map[2])):
            #     continue

            
            # if key in key_list or (a1 == a2 and np.sum(np.abs(p)) <0.001):
                # continue
            
            if abs(term.r_value_real) > 1.001*1e-33 or abs(term.r_value_imag) > 1.001*1e-33:
                # if key in Kinect_key_list:
                #     r_value_real = model.terms[key].r_value_real
                #     r_value_imag = model.terms[key].r_value_imag
                # else:
                r_value_real = model.terms[key].r_value_real
                r_value_imag = model.terms[key].r_value_imag
            # 使用外部封装好的对称化函数（注意：此处仅作为占位，实际由 builder 调用）
                ...
                Y_symm = ContinuumModelBuilder.symmetrize_Y_basis_static(term.Y_basis, k, term.symmetry_ops, symmetry_gen, term,use_cache)
                t_symm_real += time.perf_counter() - t_symm_start
                
                if abs(r_value_real) > 1.001*1e-3:
                    count_real +=1
            # if abs(term.r_value_imag) > 1:
                
                t_symm_start = time.perf_counter()
                Y_symm_imag = ContinuumModelBuilder.symmetrize_Y_basis_static(lambda k: 1j*term.Y_basis(k), k, term.symmetry_ops, symmetry_gen, term,use_cache)
                t_symm_imag += time.perf_counter() - t_symm_start
                # print(f"shape of H_cont: {H_cont.shape}, shape of Y_symm_imag: {Y_symm_imag.shape} key = {key}")
                if l1!=l2 and a1==a2 and np.linalg.norm(p)<0.001:
                    r_value_imag = r_value_imag
                    r_value_real = r_value_real
                    # if (Mz,Mz_star) in coeff_test.keys():
                    #     r_value_real = coeff_test[(Mz,Mz_star)].real/2
                    #     r_value_imag = coeff_test[(Mz,Mz_star)].imag/2
                    
                H_cont += r_value_real * Y_symm#[np.ix_(keep, keep)]
                H_cont += r_value_imag * Y_symm_imag#[np.ix_(keep, keep)]
                if abs(r_value_imag) > 1.001*1e-3:
                    count_imag +=1
                count += 1
                # if i == 0:
                    # print(f"Assembling term {term.key}... r = {r_value_real}, i = {r_value_imag}" )
            # contribution = term.r_value_real * Y_symm + term.r_value_imag * Y_symm_imag
            # H_cont += contribution
    ...
    if PROFILE_LIGHT:
        profile = {
            "t_total": t_total,
            "t_loop": t_loop,
            "t_symm_real": t_symm_real,
            "t_symm_imag": t_symm_imag,
            "t_schur": t_schur,
            "t_eig": t_eig,
            "n_terms_used": count,
        }
        return H,eigvals,vec,[count,count_real,count_imag],profile
    return H,eigvals,vec,[count,count_real,count_imag]

results_new_ = Parallel(n_jobs=1)(delayed(compute_eigenvalues)(i,k) for i,k in tqdm(enumerate(kpath), total=len(kpath)))
...
if PROFILE_LIGHT and len(results_new_) > 0:
    ...
    print(
        "  total={:.4f}s loop={:.4f}s symm_real={:.4f}s symm_imag={:.4f}s schur={:.4f}s eig={:.4f}s".format(
            avg["t_total"], avg["t_loop"], avg["t_symm_real"], avg["t_symm_imag"], avg["t_schur"], avg["t_eig"]
        )
    )
    print(
        "Light profiling (share of total):"
        " loop={:.1%} symm_real={:.1%} symm_imag={:.1%} schur={:.1%} eig={:.1%}".format(
            sums["t_loop"] / denom,
            sums["t_symm_real"] / denom,
            sums["t_symm_imag"] / denom,
            sums["t_schur"] / denom,
            sums["t_eig"] / denom,
        )
    )
