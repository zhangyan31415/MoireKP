...
def _required_text(payload: Mapping[str, Any], name: str) -> str:
    ...
    if value.shape != ():
        raise _reject(
            CandidateRejectionReason.HANDOFF_IDENTITY,
            f"routed Gamma handoff {name} must be scalar",
        )
    scalar = value.item()
...
def _strict_index_tuple(value: Any, *, name: str) -> tuple[int, ...]:
    ...
    if array.ndim != 1 or not np.issubdtype(array.dtype, np.integer):
        raise _reject(
            CandidateRejectionReason.HANDOFF_IDENTITY,
            f"routed Gamma {name} must be a one-dimensional integer array",
        )
...
def _strict_hash_tuple(value: Any, *, name: str, length: int) -> tuple[str, ...]:
    array = np.asarray(value)
    if array.ndim != 1 or array.dtype.hasobject or len(array) != length:
        raise _reject(
            CandidateRejectionReason.HANDOFF_IDENTITY,
...
def certify_gamma_routed_basis_spec(
    *,
    ...
    certified_identity["basis_hash"] = _bound_routed_basis_hash(
        ...
        raw_action_package_hash=provisional.raw_action_package_hash,
...
def load_gamma_routed_basis_spec(path: str | Path) -> GammaRoutedBasisSpec:
    ...
    with archive as payload:
        ...
        if any(np.asarray(payload[name]).dtype.hasobject for name in payload.files):
            raise _reject(
                CandidateRejectionReason.HANDOFF_IDENTITY,
                "routed Gamma basis archive contains object arrays",
            )
        ...
        if "source_hamiltonian_hash" in payload.files:
            artifact_identity["source_hamiltonian_hash"] = _required_text(
                payload, "source_hamiltonian_hash"
            )
        missing_identity = set(PROJECTION_ARTIFACT_IDENTITY_FIELDS) - set(
            artifact_identity
        )
        if missing_identity:
            raise _reject(
                CandidateRejectionReason.HANDOFF_IDENTITY,
                "routed Gamma projection identity is missing "
        ...
        spec = GammaRoutedBasisSpec(
            ...
            joint_band_indices=tuple(
                int(item) for item in np.asarray(payload["joint_band_indices"]).tolist()
            ),
            group_ranks=tuple(
                int(item) for item in np.asarray(payload["group_ranks"]).tolist()
            ),  # type: ignore[arg-type]
            group_offsets=tuple(
                int(item) for item in np.asarray(payload["group_offsets"]).tolist()
            ),  # type: ignore[arg-type]
            k_indices=k_indices,
            ...
            ordered_q_hashes=tuple(
                str(item) for item in np.asarray(payload["ordered_q_hashes"]).tolist()
            ),  # type: ignore[arg-type]
