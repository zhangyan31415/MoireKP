...
class BandStructureCalculator:
    ...
    def calculate_band_structure(self, path, kpoints=None):
        ...
        if self.config.TAPW:
            np.save(os.path.join(path, f"g_vec_list_{self.config.n_g}_{self.config.band_type}_{self.valley_flag}_1layer"),
                   self.TAPW_parameters.g_vec_list_K1)
            np.save(os.path.join(path, f"g_vec_list_{self.config.n_g}_{self.config.band_type}_{self.valley_flag}_2layer"),
                   self.TAPW_parameters.g_vec_list_K2)
        if self.config.C3_H:
            np.save(os.path.join(path, f"C3_matrix_{self.config.band_type}_{self.valley_flag}"),
        ...
        if vbm.size > 0:
            np.savetxt(os.path.join(path, f"band_data/band_data_VBM_{self.valley_flag}_valley{suffix}.txt"),
                      vbm, fmt='%15.11f')
        
        # Save CBM data if exists  
        if cbm.size > 0:
            np.savetxt(os.path.join(path, f"band_data/band_data_CBM_{self.valley_flag}_valley{suffix}.txt"),
                      cbm, fmt='%15.11f')
    ...
    def write_wave_function_spin(self, path):
        ...
        g_vec_list_K_1layer = np.load(path + f"/g_vec_list_{n_g}_{self.config.band_type}_{valley_flag}_1layer.npy")
        g_vec_list_K_2layer = np.load(path + f"/g_vec_list_{n_g}_{self.config.band_type}_{valley_flag}_2layer.npy")
